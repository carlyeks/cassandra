...
public class DebuggableThreadPoolExecutor extends ThreadPoolExecutor implements LocalAwareExecutorService
{
    ...
    {
        super.execute(!ExecutorLocal.Value.hasValue(values) || command instanceof LocalSessionWrapper
                      ? command
                      : new LocalSessionWrapper<Object>(command, values));
    }
    ...
    private static class LocalSessionWrapper<T> extends FutureTask<T>
    {
        ...
        {
            super(callable);
            values = ExecutorLocal.Value.forAll();
        }

        public LocalSessionWrapper(Runnable command, ExecutorLocal.Value[] values)
        {
            super(command, null);
            this.values = values;
        }
        ...
        {
            for (ExecutorLocal.Value value : values)
                value.set();
        }
